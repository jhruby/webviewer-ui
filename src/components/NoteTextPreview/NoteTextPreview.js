...
function NoteTextPreview(props) {
  ...
  return (
    <div className={noteTextPreviewClass} ref={ref} style={style} aria-live="polite">
      {beforeContent()}
      {renderRichText && richTextStyle ? renderRichText(textToDisplay, richTextStyle, 0) : textToDisplay} {showPrompt && (copyEnabled || !limitAnnotationHighlightText  || comment) && <button className="note-text-preview-prompt" onClick={onClickHandler}>{prompt}</button>}
    </div>
  );
}
