  ...
  return (
    <div className={noteTextPreviewClass} ref={ref} style={style} aria-live="polite">
      ...
      {renderRichText && richTextStyle
        ...
        : textToDisplay} {showPrompt && (copyEnabled || !limitAnnotationHighlightText  || comment) && <button className="note-text-preview-prompt" onClick={onClickHandler}>{prompt}</button>}
    </div>
  );
}
